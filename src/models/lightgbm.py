...
def lightgbm():
    ...
    for subset in subsets:
        ...
        model_lat = lgb.LGBMRegressor(**config.LGBM_PARAMS)
        model_lat.fit(
            X_train, y_train_lat,
        ...
        model_lon = lgb.LGBMRegressor(**config.LGBM_PARAMS)
        model_lon.fit(
            X_train, y_train_lon,
        ...
        pred_lat = current_lat_test + pred_delta_lat
        pred_lon = current_lon_test + pred_delta_lon

        true_lat = current_lat_test + y_test_lat.values
        true_lon = current_lon_test + y_test_lon.values

        test_errors_km = metrics.haversine_distance(true_lat, true_lon, pred_lat, pred_lon)
