...
class MultimodalStormDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tab_seq = np.nan_to_num(tab_seq, nan=0.0)
        
        # Normalize Tabular and Vision to prevent gradient explosion and NaN loss
        grid_seq = np.nan_to_num(grid_seq, nan=0.0)
        
        # Z-score normalization for vision data (per sample approximation)
        mean_v = grid_seq.mean(axis=(0, 2, 3), keepdims=True)
        std_v = grid_seq.std(axis=(0, 2, 3), keepdims=True) + 1e-5
        grid_seq = (grid_seq - mean_v) / std_v
        
        # Z-score normalization for tabular data (per sample approximation)
        mean_t = tab_seq.mean(axis=0, keepdims=True)
        std_t = tab_seq.std(axis=0, keepdims=True) + 1e-5
        tab_seq = (tab_seq - mean_t) / std_t
