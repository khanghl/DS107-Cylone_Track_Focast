...
from module.normal import IndependentForecastingStrategy, EnsembleForecastingStrategy, StackingForecastingStrategy
from module.cliper import CLIPERForecastingStrategy
from rich.console import Console
from rich.table import Table
from evaluation.metrics import haversine_distance, evaluate_advanced_metrics, calculate_mae_r2
from sklearn.preprocessing import StandardScaler
from sklearn.decomposition import PCA
from module.mlp_strategy import MLPForecastingStrategy
...
def run_experiment_matrix():
    ...
    scenarios = [
        ...
        {
            "name": "8. Ensemble (LightGBM + XGBoost) on ibtracs_feature_engineering + 3D",
            "strategy": "ensemble",
            "features": raw_cols + eng_cols + vis_cols
        },
        {
            "name": "9. Stacking (LightGBM + XGBoost) on ibtracs_feature_engineering + 3D",
            "strategy": "stacking",
            "features": raw_cols + eng_cols + vis_cols
        }
    ]
    ...
    for scenario in scenarios:
        ...
        table.add_column("Horizon", justify="center", style="cyan", no_wrap=True)
        table.add_column("Distance (km)", justify="center", style="magenta")
        table.add_column("Dir. Stability", justify="center", style="green")
        table.add_column("Trk MAE", justify="center", style="yellow")
        table.add_column("Trk R2", justify="center", style="yellow")
        table.add_column("Wind MAE", justify="center", style="red")
        table.add_column("Wnd R2", justify="center", style="red")
        
        # Train and Predict depending on the strategy
        if strategy == "cliper":
            runner = CLIPERForecastingStrategy()
            runner.train(X_train, y_train_lat, y_train_lon, y_train_wind)
            pred_lat, pred_lon, pred_wind = runner.predict(X_test)
            ...
            runner.train(X_tr, y_train_lat, y_train_lon, y_train_wind, X_va, y_val_lat, y_val_lon, y_val_wind)
            pred_lat, pred_lon, pred_wind = runner.predict(X_te)
            
        else: # Standard LightGBM or Ensemble
            ...
            if strategy == "ensemble":
                runner = EnsembleForecastingStrategy(['lightgbm', 'xgboost'])
            elif strategy == "stacking":
                runner = StackingForecastingStrategy(['lightgbm', 'xgboost'])
            else:
                runner = IndependentForecastingStrategy('lightgbm')
        ...
        for h in config.HORIZONS:
            ...
            directional_stability, velocity_sensitivity = evaluate_advanced_metrics(y_test_actual, y_test_predicted, v_past, verbose=False)
            
            # Calculate MAE and R2 for trajectory
            mae, r2 = calculate_mae_r2(y_test_lat[h], y_test_lon[h], pred_lat[h], pred_lon[h])
            
            # Calculate MAE and R2 for wind intensity
            mae_wind = mean_absolute_error(y_test_wind[h], pred_wind[h])
            r2_wind = r2_score(y_test_wind[h], pred_wind[h])
            
            table.add_row(
                f"+{h}h",
                f"{err_mean:.2f}",
                f"{directional_stability:.2f}%",
                f"{mae:.4f}",
                f"{r2:.4f}",
                f"{mae_wind:.4f}",
                f"{r2_wind:.4f}"
            ...
            report_rows.append({
                ...
                "MAE": round(mae, 4),
                "R2": round(r2, 4),
                "Wind_MAE": round(mae_wind, 4),
                "Wind_R2": round(r2_wind, 4),
            ...
            traj_df['TRUE_TARGET_WIND'] = y_test_wind[h].values
            traj_df['PRED_TARGET_WIND'] = pred_wind[h]
