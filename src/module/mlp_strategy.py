...
from module.mlp import StormMLP
...
class MLPForecastingStrategy:
    ...
    def train(self, X_train, y_train_lat_dict, y_train_lon_dict, y_train_wind_dict, X_val, y_val_lat_dict, y_val_lon_dict, y_val_wind_dict):
        ...
        for horizon in config.HORIZONS:
            ...
            y_tr_wind = y_train_wind_dict[horizon].values.astype(np.float32)
            y_tr_stacked = np.column_stack((y_tr_lat, y_tr_lon, y_tr_wind))
            
            y_va_lat = y_val_lat_dict[horizon].values.astype(np.float32)
            y_va_lon = y_val_lon_dict[horizon].values.astype(np.float32)
            y_va_wind = y_val_wind_dict[horizon].values.astype(np.float32)
            y_va_stacked = np.column_stack((y_va_lat, y_va_lon, y_va_wind))
    ...
    def predict(self, X_test):
        ...
        for horizon in config.HORIZONS:
            ...
            predictions_wind[horizon] = preds[:, 2]
